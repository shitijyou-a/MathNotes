...
\section{Saturated Ideals and Closed Sub-Schemes.} %% Ex5.10 
    ...
    \begin{Remark}
        ...
        あるいは次のようにも表現できる．
        \[ \bigcup_{i=0,\dots,n} \bigcup_{r>0} (I:x_i^r) \subseteq I. \]
    \end{Remark}
    ...
    \subsection{$\bar{I}_1=\bar{I}_2 \iff \Proj S/I_1 \iso \Proj S/I_2$.}
    

    \subsection{The Ideal $\Gamma_*(\shI_Y)$ is saturated.}

    \subsection{Saturated Ideals $\leftrightarrow$ Closed Subschemes of $X=\Proj S$.}
...
\section{The $d$-uple Embedding is Projectively Normal.} %% Ex5.14 
    ...
    \paragraph{$\Forall{f \in S_1} S_{(f)}$ :: domain $\implies$ $S$ :: domain.}
    $S$ :: reducedから示す．
