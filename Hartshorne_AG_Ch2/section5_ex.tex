...
\section{The $d$-uple Embedding is Projectively Normal.} %% Ex5.14 
    ...
    $n \geq 0$だから
    \[ \shJ_x=\left\{ \frac{a}{f} \in S_{\I{p}_x}~\middle|~ f\text{ :: homogeneous}, \deg a \geq \deg f \right\}. \]
    ここでの$\deg$は$S$に付与されているものである．
    ...
    \]
    $d=\deg a/f<0$の時，$\deg u_i/v_i \geq 0$より左辺の次数は有限負になる．
    したがって$d \geq 0$．
    $f \in S-\I{p}_x$かつhomogeneousであることを確かめよう．
    これは$S_{(\I{p}_x)}$ :: integrally closedを利用する．
    まず，$\I{p}_x$が$S_1$全体を含むことはない
    （そうなれば$S_+ \subseteq \I{p}_x$となるが，これは$\Proj S$の定義からありえない）．
    なので$u \in (S-\I{p}_x)_1$がとれる．
    上の等式から$rd$次斉次成分を取り出し，$u^{-rd}$を両辺にかけても正しい等式となる．
    \[
        \left(\frac{a_d}{u^df}\right)^r+
        \frac{1}{u^d}\left(\frac{u_{r-1}}{v_{r-1}}\right)_{d}\left(\frac{a_d}{u^df}\right)^{r-1}+
        \dots+
        \frac{1}{u^{rd}}\left(\frac{u_{0}}{v_{0}}\right)_{rd}
        =0
    \]
    すると，$a_d/u^df$及び係数の次数は$0$になる．
...
%    斉次分解を行えば，$f, \{c_i\}_{i=0}^{r-1}$は斉次元として考えても良いことが分かる．
    ...
    この全射の$\ker$を$I$とすれば$S_A^r/I \iso \Gamma_*(\shO_X)=S'$．
    $S$の定義（特に$\shI_X$の定義）から$S=S_A^r/I \iso S'$．
...
\section{Tensor Operations on Sheaves.} %% Ex5.16 
    ...
    また，$I ~(\#I=r)$の元を小さいものから並べた列$i_1<\dots<i_{n-r}$に対し，
    列$1<\dots<n$から（順序を保って）$i_1<\dots<i_{n-r}$を
    取り除いたものを$i_{n-r+1}<\dots<i_n$とする．
    $S=\{1,2,3,4,5\}, I=\{2,3,5\}$なら$i_4=1, i_5=4$である．
    そこで$\sigma_{I} \in S_n$を$\sigma_I(j)=i_j$で定める．
    ...
    $I \cap J = \emptyset$ならば$\mu(x_I, x_J)$は$\pm 1 \in A$へ写る．
    ...
    $I \subseteq S$について，
    $x_I \wedge x_{I^c}=(\sgn \sigma) x_S$となる．
    $\mu(x_I, x_J) \neq 0$となるのは$J=I^c$の時のみ．
    なので$x_I$の像$\mu(x_I,*) \otimes (x_{I^c} \wedge x_I)$の像は，
    上のように定めた$\sigma$について$(\sgn \sigma)^2 x_I=x_I$．
    逆に$\phi \otimes x_S$の像
    $\sum_I (\sgn \sigma_I) \phi(x_I)x_{I^c}$の像は，
    $\phi=\sum_I (\sgn \sigma_I) \phi(x_I)\mu(x_{I^c}, *)$ゆえに$\phi \otimes x_S$．
