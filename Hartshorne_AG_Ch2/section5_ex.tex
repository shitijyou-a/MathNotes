...
\section{The $d$-uple Embedding is Projectively Normal.} %% Ex5.14 
    ...
    上の等式の両辺に$s^{-rd}$を両辺にかけて，$0$次斉次成分を取り出しても正しい等式となる．
    \[
        \left(\frac{a_d}{s^df}\right)^r+
        \left(\frac{1}{s^d}\frac{u_{r-1}}{v_{r-1}}\right)_0\left(\frac{a_d}{s^df}\right)^{r-1}+
        \dots+
        \left(\frac{1}{s^{rd}}\frac{u_{0}}{v_{0}}\right)_0
        =0
    \]
    すると，$a_d/s^df$及び係数の次数は$0$になる．
