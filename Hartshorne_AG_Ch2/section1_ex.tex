...
\section{Adjoint Property of $f^{-1}$.} %% Ex1.18 
    ...
    \[ \eta: \id{\Sh(Y)} \to f_* f^{-1}, \epsilon: f^{-1}f_* \to \id{\Sh(X)} \]を定義し，
    sheaf $\shF, \shG$について$\Hom(f^{-1}\shG, \shF) \cong \Hom(\shF, f_*\shF)$を示す．

    \paragraph{Construction of $\tilde{\eta}, \eta$.}
    \paragraph{$\tilde{\eta}, \eta$ is Natural.}
    \paragraph{Construction of $\tilde{\epsilon}, \epsilon$.}
    \paragraph{$\tilde{\epsilon}, \epsilon$ is Natural.}
    \paragraph{Construction of $\Phi_{\shF, \shG}, \Psi_{\shF, \shG}$.}
    $\Phi_{\shF, \shG}: \Hom(\shG, f_* \shF) \leftrightarrow \Hom(f^{-1}\shG, \shF): \Psi_{\shF, \shG}$
    を構築する．
    ...
    \[
        ...
        {
        {} & \shF  & {} \\
        f^{-1} \shG \ar@{~>}[ru]^-{\bar{\alpha}} \ar[rr]^-{f^{-1} \alpha} & {} & f^{-1} f_* \shF \ar[lu]_-{\epsilon_{\shF}} \\
        \shG \ar[rr]^-{\alpha} & {} & f_* \shF \\
        }
        \hspace{1em}
        (3)
        ...
        f_* f^{-1} \shG \ar[rd]_-{f_* \bar{\alpha}} \ar[rr]^-{f_* f^{-1} \alpha} & {} & f_* f^{-1} f_* \shF \ar[ld]^-{f_* \epsilon_{\shF}} \\
        {} & f_* \shF  & {} \\
        }
    \]
    図式(1)を$f^{-1}$で写したものに，$\shF$と$\epsilon_{\shF}$を追加すると，射の結合として$\bar{\alpha}$が得られる．
    これが図式(2)であり，
    \[ \Phi_{\shF, \shG}(\alpha)=\bar{\alpha}=\epsilon_{\shF} \circ f^{-1} \alpha \in \Hom(f^{-1}\shG, \shF) \]
    と定義する．
    さらに(2)の上の三角形を$f_*$で写したものが図式(3)の下の三角形である．
    $\eta$の自然性から，$\eta$を追加した図式(3)も可換である．
    すると$\bar{\alpha} \in \Hom(f^{-1}\shG, \shF)$から
    ...
    \[
        ...
        {} & {} & f_* f^{-1} f_* \shF \ar[ld]^-{f_* \epsilon_{\shF}} \\
        {} & f_* \shF  & {} \\
        }
    \]
    $f_* f^{-1} f_* \shF$は$f_* f^p f_* \shF$のsheafificationである．
    ...
    \xymatrix
    {
    (f_* f^p f_* \shF)(U) \ar[rr]^-{(\tilde{\epsilon}_{f_*\shF})_U}& {} & \shF(f^{-1}(U)) \\
    \mathscr{D}_1 \ar[u] \ar[urr]^(.2){\res}
    & \ni 
    & f_*\shF(U) \ar@{=}[u]_-{\res_{f^{-1}(U)}^{f^{-1}(U)}=\id{}} \ar[ull]_(.2){(f_*\tilde{\eta}_{\shF})_U}
    }
    \]
    ...
    これは直ちに$\tilde{\epsilon}_{f_*\shF} \circ f_*\tilde{\eta}_{\shF}=\id{}$を意味する．
    ここまでは$f_* f^p f_*\shF$についての議論だったが，
    これをsheafificationすれば最初の目的の等式が得られる．

    \paragraph{$\Phi_{\shF, \shG} \circ \Psi_{\shF, \shG}=\id{\Hom(f^{-1}\shG, \shF)}$.}
    今度は$\beta \in \Hom(f^{-1}\shG, \shF)$をとる．
    ...
    \[
        ...
        f_* f^{-1}\shG \ar[rr]^-{f_* \beta} & {} & f_* \shF \\
        f^{-1} \shG \ar[rr]^-{\beta} & {} & \shF \\
        f^{-1} f_* f^{-1} \shG \ar[u]^-{\epsilon_{f^{-1} \shG}}  \ar[rr]^-{f^{-1} f_* \beta} & {} & f^{-1} f_* \shF \ar[u]_-{\epsilon_{\shF}} \\
        {} & f^{-1} \shG \ar[ru]_-{f^{-1} \bar{\beta}} \ar[lu]^-{f^{-1} \eta_{\shG}} & {} \\
        }
    \]
    よって以下が$\id{f^{-1}\shG}$であることを示せば良いと分かる．
    \[
        \xymatrix
        {
        f^{-1} \shG & {} & {} \\
        f^{-1} f_* f^{-1} \shG \ar[u]_-{\epsilon_{f^{-1} \shG}} & {} & {} \\
        {} & f^{-1} \shG\ar[lu]^-{f^{-1} \eta_{\shG}} & {} \\
        }
    \]
    そこで最初に次の図式を考える．
    \[
        \xymatrix@C=5em
        {
        (f^p \shG)(U) \ar[r]^-{(f^p \tilde{\eta}_{\shG})_U}
        & (f^p f_* f^p \shG)(U) \ar@{-->}[r]^-{(\tilde{\epsilon}_{f^p\shG})_U}
        & (f^p \shG)(U) \\
    ...
    ここから当初の予定の等式を得るためには，前段落の場合と違って注意が必要である．
    なぜなら，前段落では$f_*f^{-1}f_*\shF$は$f_*f^p f_*\shF$を一回sheafificationしたものだが，
    $f^{-1} f_* f^{-1} \shG=\ftorSh f^p f_* \ftorSh f^p \shG$は
    2回sheafificationを行っているからである．
    だから，議論は次のように行う．
    次の可換図式から始める．
    \[
        \xymatrix
        {
        \shG \ar[r]^-{\eta_{\shG}} \ar[rd]_-{\tilde{\eta}_{\shG}}
        & f_* f^{-1} \shG
        \\
        {}
        & f_* f^p \shG \ar[u]_-{\theta}
        \\
        }
    \]
    この可換図式を$f^p$で写し，
    $\tilde{\epsilon}_{f^{-1}\shG}, \tilde{\epsilon}_{f^p\shG}$を
    追加して，もう一度sheafificationすると，以下の可換図式が得られる．
    \[
        \xymatrix@C=5em
        {
        f^{-1}\shG \ar[r]^-{f^{-1}\eta_{\shG}}
        & f^{-1} f_* f^{-1} \shG \ar[rd]^-{\epsilon_{f^{-1}\shG}}
        {} \\
        f^p \shG \ar[r]^-{f^p \eta_{\shG}} \ar[rd]_-{f^p \tilde{\eta}_{\shG}} \ar[u]^-{\theta}
        & f^p f_* f^{-1} \shG \ar[r]_-{\tilde{\epsilon}_{f^{-1}\shG}} \ar[u]
        & f^{-1} \shG \\
        {} 
        & f^p f_* f^p \shG \ar[r]_-{\tilde{\epsilon}_{f^p\shG}}  \ar[u]
        & f^p \shG \ar[u]_-{\theta} \\
        }
    \]
    この可換図式の上の辺は，
    一番下の辺（今$\id{}$であることを示した）を元に，
    $f^p \shG$のsheafificationのUMPを使って作り出したものに等しい．
    $\id{}$から出来た射なので，一番上の辺は$\id{}$．
    （$\ftorSh$と$f_*$が交換できることについては書いていない．）
