...
\section{Espace \'Etal\'e of a Presheaf.} %% Ex1.13 
    ...
    \'etal\'e space over $X$をとって，
    その連続なsection全体をとる関手を
    $\ftorSec: \mathbf{Et}(X) \to \Sh(X)$とする．
    逆にpresheafから\'etal\'e space over $X$を作る関手を
    $\ftorEt: \PSh(X) \to \mathbf{Et}(X)$とする．
    sheafification functorが$\ftorSh=\ftorSec \ftorEt$で定義できることを示す．

    \paragraph{Plan of Proof.}
    二つの写像を定める．
    $\shF$ :: sheaf on $X$,
    $E$ :: \'etal\'e space over $X$,
    $U$ :: open subset of $X$,
    $P \in U$
    とする．
    \begin{defmap}
        \alpha_{\shF}:& \shF& \to& \ftorSec \ftorEt \shF \\ 
        \vspace{2ex}
        {}& \sect{U}{s}& \mapsto& \bar{s} \\
        \beta:& \ftorEt \ftorSec E& \to& E \\ 
        {}& (\ftorSec E)_P \ni \sigma_P & \mapsto& \sigma(P) \\
    \end{defmap}
    $\sect{U}{s} \in \shF$は$s \in \shF(U)$という意味である．
    $\bar{s} : U \to \ftorEt \shF, \sigma: U \to E$に注意．
    ...
    $\sigma: U \to \ftorEt \shF=\bigcup_{P \in X} \shF_P$なので，
    以下が成り立つ．
    \[
        \Forall{P \in U}
        P \in {}^{\exists} V \subseteq U \text{ :: open},~~
        \Exists{s \in \shF(V)} \sigma(P)=s_P(=\bar{s}(P) \in \shF_P).
    \]
    ...
    \footnote
    {
        $\bar{s}$の像位相において，
        開集合$V$の像が開集合であることは
        $\bar{s}^{-1}(\bar{s}(V))$が開集合であることと同値だが，
        単射性から，$\bar{s}^{-1}(\bar{s}(V))=V$．
    }．
    しかも$\sigma$ :: continuousだから，
    $\sigma(S) \subseteq \alpha(s)(V)$なる
    $P \in S \subseteq \sigma^{-1}(\alpha(s)(V))$ :: openが存在する
    \footnote
    {
        これは$\epsilon$-$\delta$論法に似ている．
        $\sigma^{-1}(\alpha(s)(V))$が開集合であるから，
        任意の点，特に$P$はこの集合の内点である．
        このことから開集合$S$が存在することは
        開集合のよく知られた性質である．
    }．
    $\sigma(S) \subseteq \alpha(s)(V)$より
    直ちに以下が成り立つ．
    \[ \Forall{Q \in S} \Exists{Q' \in V} \shF_Q \ni \sigma(Q)=\alpha(s)(Q')(=s_{Q'}) \in \shF_{Q'}． \]
    $\shF_{Q} \cap \shF_{Q'} \neq \emptyset$ということなので$Q=Q'$，
    よって$\sigma|_S=\alpha(s)|_S$が成り立つ．
    以上から，ある点で$\sigma$と$\alpha(s)$が一致するならば，
    その点の開近傍でも両者は一致する．
    さらに点$P$を様々に取ることで，$S$で$U$を被覆できることがわかる．
    開集合$S \subseteq U$において$\sigma|_S=\alpha(s)|_S$，
    開集合$T \subseteq U$において$\sigma|_T=\alpha(t)|_T$であるとする．
