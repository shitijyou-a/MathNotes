...
\section{Surjectivity of Morphism is Local Property.} %% Ex1.3 
...
%    そして$\shF^i(U_1) \oplus \dots \oplus \shF^i(U_n)$として，
%    これがdirect limitのUMPを満たすことを見る．)
%    \paragraph{Identity Axiom.}
%    $s \in \shL(U)$が任意の$\lambda$について$s|_{U_{\lambda}}=0$を満たすとしよう．
%    以下の可換図式を見よ．
%    \[
%    \xymatrix
%    {
%    \shL(U) \ar@{-->>}[r]^-{\res_{U}^{U_{\lambda}}} & \shL(U_{\lambda}) \\
%    \shF^i(U) \ar[u] \ar@/_4mm/[ur] \ar[r]_-{f^{ij}_{U}}& \shF^j(U) \ar[u] \ar@/^4mm/[ul]
%    }
%    \]
%    $\shF^*(U)$から$\shL(U_{\lambda}), \shL(U)$へのmorphismが存在することは
%    $\shL$が$\PSh(X)$でのdirect limitであることから明らか．
%    射$\res_{U}^{U_{\lambda}}$はepi(=surj)．
%    したがって$s|_{U_{\lambda}}=0$より$s|_{U}=0$が得られる．

%    \paragraph{Gulability Axiom.}
%    $s_{\lambda} \in \shL(U_{\lambda})$であるsectionの集合$\{s_{\lambda}\}$をとる．
%    これらが任意の$\lambda, \mu$について
...
%    \subsubsection{Another Sheafification.}
%    $\shF \in \PSh(X)$に対し，そのsheafification $\shF^+$は以下を満たすことを示す．
%    \[ \Forall{U \subseteq X} \shF^+(U)=\text{continuous sections of $\Spe(\shF)$ over $U$}=:\shF^@(U). \]
%    $\shF^@(U)$の元は$\shF(U)$の元から得られるものだけではないことに注意せよ．

%    $U$ :: open in $X$をひとつ取って固定する．
%    $s \in \shF^+(U), \sigma \in \shF^@(U)$による
%    $P \in U$の像がどちらの場合も$\shF_P$に入ることは，定義から明らか．
%    $\sigma$が満たすべき定義を書き下すと，以下のようになる．
%    \[
%        \left[ \pi \circ \sigma=\id{U} \right]
%        \land
%        \left[ \Forall{\tilde{V} \OpenIn \Spe(\shF)} \sigma^{-1}(\tilde{V}) \OpenIn U \right].
%    \]
%    $\pi \circ \sigma=\id{U}$は$\sigma(P) \in \shF_P$を意味する．
%    後半の条件を書き換えていく．
%    \begin{align*}
%        {}&     \Forall{\tilde{V} \OpenIn \Spe(\shF)} \sigma^{-1}(\tilde{V}) \OpenIn U, \\
%        \iff&   \Forall{\tilde{V} \in \bigcap_{t \in \shF(U)} \bar{t}^{-1}(\mathcal{O}_U)}
%                \sigma^{-1}(\tilde{V}) \OpenIn U, \\
%        \iff&   \Forall{\tilde{V} \in \bigcap_{t \in \shF(U)}\{ \{t_P ~|~ P \in V\} ~|~ V \OpenIn U. \} } \\
%        {}&     \hspace{8em} \Exists{W \OpenIn U} W=\sigma^{-1}(\tilde{V}), \\
%        \iff&   \Forall{\tilde{V} \in \bigcap_{t \in \shF(U)}\{ \{t_P ~|~ P \in V\} ~|~ V \OpenIn U. \} } \\
%        {}&     \hspace{8em} \Exists{W \OpenIn U} \Forall{Q \in W} \Exists{\tilde{R} \in \tilde{V}} \sigma(Q)=\tilde{R}.
%    \end{align*}
%    ここで現れる$\tilde{V}$は$V$を指定すれば一意に定まる．
%    (正確には$\Forall{\tilde{V}} \Exists{V} V=\tilde{V}$で，中身まで特定すると$\Forall{\tilde{V}} \Exists{V} \Exists{t} V=\{t_P ~|~ P \in V\}$．)
%    $V$は任意の$U$の開集合だから，さらに次のように書くことが出来る．
%    \begin{align*}
%        {}&     \Forall{V \OpenIn U} 
%                \Exists{W \OpenIn V} \Forall{Q \in W} \Exists{t \in \shF(W)} \Exists{\tilde{R} \in \{t_P ~|~ P \in V\}} \sigma(Q)=\tilde{R}, \\
%        \iff&   \Forall{V \OpenIn U}
%                \Exists{W \OpenIn V} \Forall{Q \in W} \Exists{t \in \shF(W)} \sigma(Q)=t_Q, \\
%        \iff&   \Forall{V \OpenIn U} 
%                \Exists{W \OpenIn V} \Exists{t \in \shF(W)} \Forall{Q \in W} \sigma(Q)=t_Q.
%    \end{align*}
%    $\Exists{t \in \shF(W)}$は$Q \in W$に依存していないので順番を入れ替えた．
%    $V,W$を指定する部分は，要するに$U$のcoverの中に条件を満たす開集合$W$が存在する，ということ．
%    だから最終的に次が得られる．
%    \[ \Forall{P \in U} (P \in)~ \Exists{V \OpenIn U} \Exists{t \in \shF(V)} \Forall{Q \in V} \sigma(Q)=t_Q. \]
%    これは$\shF^+$の元の定義である．
...
%    $U \OpenIn X$について，
%    $\ftorSec$は$\Hom(U,\Spe(-))$から$\Hom(U,U)$への，次の二つの射のequalizerである．
%    \[ (\pi \circ): f \mapsto \pi \circ f,~~ \overline{\id{U}}: * \mapsto \id{U}. \]
%    後者はすなわち定値写像である．
%    また，$\ftorEt$は次のdirect systemについてのdirect limitである．
%    \[ \text{For } U,V \OpenIn X, s \in \shF(U) \text{ and } t \in \shF(V),~~[U \supseteq V] \land [s|_{V}=t] \iff U \times s \geq V \times t \]
%    先に定めた$\Spe \shF$と$\ftorEt \shF$が一致することは，二つが同じUMPを持つことから分かる．
%    この場合，前者は終位相のUMPを持ち，後者はdirect limitのUMPを持つ．
...
%    abelian group $\shF(U)$を以下で定める．
%    \[ \shF(U)=\left( {\bigsqcup_{i \in I} \{i\} \times \shF_i(U \cap U_i)}\right) \Big/ {\sim}. \]
%    ただし$\sim$は以下で定まる同値関係である．
%    \[ (i, s) \sim (j, t) \iff (\phi_{ij})_{U \cap U_i \cap U_j}(s)=t. \]
%    $s \in \shF_i(U \cap U_i)$に注意せよ．
%    これが同値関係であることは自明なので証明を略す．

%    \subsubsection{$\shF(U)$ :: abelian group.}
%    任意の2元$(i,s), (j,t) \in \shF(U)$に対し，
%    $(j,t)$が属す同値類から$(i,t') \in \{i\} \times \shF_i(U \cap U_i)$が取れる．
%    これは$\phi_{ij}: \shF_i|_{U_i \cap U_j} \isomap \shF_j|_{U_i \cap U_j}$が全単射（同型）だからである．
%    この時，$(i,s)+(j,t)$は$(i, s+t')$で定義できる．
%    可換性は$\shF_*(U \cap U_*)$の元が可換であることから得られる．
%    単位元は$(i, 0)$である．

%    \subsubsection{$\shF$ :: sheaf.}
%    \subsubsection{$\shF|_{U_i} \iso \shF_i$.}
