...
\section{Adjoint Property of $f^{-1}$.} %% Ex1.18 
    ...
    coconeの「底面の辺」と「母線」の結合として
    \[ (\eta^{pre}_{\shG})_U: \shG(U) \to \shG(f \circ f^{-1}(U)) \to (f_* f^{-1} \shG)^{pre}(U) \]
    が得られる．
    ...
    $V \supseteq f(U)$であるとき，$f^{-1}(V) \supseteq f^{-1} \circ f(U) \supseteq U$．
    したがってcolimitのUMPにより$(\epsilon^{pre}_{\shF})_U$が得られる．
    \[
    \xymatrix
    {
    (f^{-1} f_* \shF)^{pre}(U) \ar@{-->}[r]^-{(\epsilon^{pre}_{\shF})_U}& \shF(U) \\
    \dots \ar[r]_-{\res} \ar[u] \ar@/_10pt/[ur]& \shF(f^{-1} \circ f(U)) \ar[u]_-{\res} \ar@/^10pt/[ul]
    }
    \]
...
%    なお，$\Supp (s|_{V_i})=\emptyset$から$\Supp (s|_V)=\emptyset$が得られる．
    ...
    Ex1.2cを応用するために，先に$\mathscr{H}_Z^0(\shF)$のstalkを考えよう．
