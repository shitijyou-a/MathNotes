...
\begin{Remark}
    ...
    さらに$\xi, \eta \in \shF(U)$をとり，$\epsilon_{\covU}$による像を
    \[
...
\begin{Lemma}
    ...
    特に，$\shF(\emptyset)$の任意の二つの対象の間には，ただ一つの同型射が存在する．
\end{Lemma}
\begin{proof}
...
\begin{proof}
    ...
    $i \neq j$の時，
    $\sigma_{ij}$は$\shF(U_{ij})=\shF(\emptyset)$の射であるから，
    ...
    \begin{defmap}
        {} & \mathrm{Ob}\left( \prod_i \shF(U_i) \right)& \to&
