...
\begin{Def}[universal, uniform quotient]
    $q \colon X \to Y$を上記のいずれかのquotientとする．

    \begin{itemize}
    \item 
        任意の射$Y' \to Y$によるpullback :: $q' \colon X \times_Y Y' \to Y'$も
        同じ種類のquotientであるとき，$q$はuniversalであると言う．
    \item
        任意のflat射$Y' \to Y$によるpullback :: $q' \colon X \times_Y Y' \to Y'$も
        同じ種類のquotientであるとき，$q$はuniformであると言う．
    \end{itemize}
\end{Def}
...
\begin{Prop}[\cite{Rydh13}, Prop2.3]
    ...
    この時，(iii) $\implies$ (ii) $\iff$ (i)がそれぞれ成り立つ．
    さらに$q, j_{/Y}$ :: locally of finite type or integralならば，
    (iii) $\impliedby$ (ii)も成り立つ．
\end{Prop}
\begin{proof}
\end{proof}
...
\begin{Lemma}
    Remark 2.5
    when univ. Zariski quot. is top. quot.?
\end{Lemma}
