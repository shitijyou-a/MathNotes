...
\begin{proof}
    ...
    細分の定義から，各$i,k$について以下が可換に成る射$\iota_{ik} \colon V_{ik} \to V_{i}$が存在する．
    \[
    \begin{tikzcd}
        V_{ik} \ar[r, "\iota_{ik}"'] \ar[rr, bend left, "\psi_{ik}"]& V_{i} \ar[r, "\phi_i"']& U
    \end{tikzcd}
    \]
    この射$\iota_{ik}$を用いて，
...
        \mathbf{Arrows}& \{\alpha_i\}& \mapsto& \{(\iota_{ik})^*\alpha_i\}
    ...
    \begin{tikzcd}[row sep=1.4cm, column sep=0.7cm]
        ...
        {}& V_i \times_U V_j \ar[r, "\pr_1"]\ar[d, "\pr_2"']& V_i \ar[d, "\phi_i"']\\
        V_{jl} \ar[r, "\iota_{jl}"]\ar[rr, bend right, "\psi_{jl}"']& V_{j} \ar[r, "\phi_j"]& U
    \end{tikzcd}
    \end{center}
    ...
    \footnote
    {
        ...
        射$V_{il} \times V_{jm} \times V_{kn} \to V_{i} \times V_{j} \times V_{k}$を用いれば良い．
    }．
