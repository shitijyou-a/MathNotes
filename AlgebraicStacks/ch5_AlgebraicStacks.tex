...
\subsection{Representable Ones}
    ...
    \begin{Remark}
        ...
        \cite{VerDefAlgSt}, \cite{StacksProj}ではfppf siteを考え，
        \cite{ChAlg}, \cite{ASS}ではetale siteを考える．
    \end{Remark}
    \rest
...
\begin{Def}[\rep Property of Morphism of Algebraic Stack, \cite{ChAlg} p.33, \cite{IrrOfMg} p.100]
    $\mathcal{P}$をmorphism of schemeの性質であって，
    smooth local on source and targetであるものとする．
    あるいは，DM stackを考えるならば
    etale local on source and targetであるものとする．
    以下の可換図式で，$V \to \fibY, U \to V \times \fibX$はatlasであるとする
    \footnote
    {
        すなわち，
