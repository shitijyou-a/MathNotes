...
\section{Linear Representation of Group Scheme.}
    ...
    \begin{Def}[\cite{MilneiAG} 4,a]
        ...
        で定める．
        $G$の\textbf{linear representation} とは，
        $V$と（群の圏への関手の）
    ...
    \begin{Def}[\cite{MilneiAG} 4,aにある別の定義]
        $V$ :: vector space over $k$に対し，
        $k$代数の圏から加群の圏への関手$\func{V}$を
        \[ R \mapsto V \otimes_k R \]
        で定める．
        $G$のlinear representationとは，
        $V$と，
        次の条件を満たす自然変換
        $\rho: \func{G} \times \func{V} \to \func{V}$の2つ組のことである．
        その条件とはすなわち，
        任意の$k$代数$R$について
...
%    \end{Remark}
    ...
    \begin{Example}
        ...
        次のように定義される作用である．
        \begin{defmap}
            {}& \func{G}(R) \times \func{V}(R)& \to& \func{V}(R) \\
            {}& g \times v& \mapsto& v
        \end{defmap}
    \end{Example}
    ...
    \item
        $V, W$ :: vector space over $k$に対し，
        $G$のrepresentationが存在するとする．
        \[
    ...
    \begin{Remark}
        $k$-algebra :: $A$をとる．
        $A$を$k$-vector spaceと考えると，
        $G$から$\Spec A$への作用は$G$のrepresentationと考えることが出来る．
...
\section{Affine GIT Quotient of Variety is Variety Again }
    ...
    \begin{Prop}[\cite{Muk1} Thm4.51, \cite{Hos} Thm4.25 (first part)]
