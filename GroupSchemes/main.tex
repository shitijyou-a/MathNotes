...
%            $X \times S'$の$G \times S'$によるcategorical quotientである．
    ...
    \begin{Def}[\cite{Hos}] \label{def:good_quotient}
...
\section{Linear Representation of Group Scheme.}
    ...
    \begin{Def}[\cite{MilneAG} 4,aにある別の定義]
        $V$ :: vector space over $k$に対し，
    ...
    \begin{Prop}[\cite{Muk1} p.131]
        $G$とそのnormal subgroup $H$
        % $\func{H}(T)$が$\func{G}(T)$のnormal subgroup
        を考える．
...
\section{Affine GIT Quotient of Variety is Variety Again }
    ...
    \begin{Prop}[\cite{Muk1} Thm4.51, \cite{Hos} Thm4.25 (second part)]
...
\section{Affine GIT Quotient is a Good Quotient}
    ...
    \begin{proof}
        ...
        $G$ :: linearly reductiveより，
        \begin{defmap}
            {}& (\I{a}_1 \cap A^G) \oplus (\I{a}_2 \cap A^G)& \to& A^G \\
            {}& (a_1, a_2)& \mapsto& a_1+a_2
        \end{defmap}
        も全射．
...
%        \footnote
%        {
%            ここでは$X, W_1, W_2$のclosed pointsを
%            $k^{\oplus n}$の部分集合とみなしている．
%            この同一視によって$f_1, f_2 \in A$を$k^{\oplus n} \to k$の写像としている．
%            写像$f_1$はschemeの間の射$\phi_1$へと拡張できる(\cite{HarAG} II, Prop2.6).
%            また，拡張しても$\phi_1(W_1)=0, \phi_1(W_2)=1$は変わらない
%            (\cite{HarAG} II, Ex3.14 + Prop4.1 + Ex4.2).
%        }.
    \end{proof}

    主張を述べる前に，
    good quotientの定義を再掲する．
    \begin{Def}[\cite{Hos}, 定義\ref{def:good_quotient}の再掲]
        scheme morphism :: $q: X \to Y$が
